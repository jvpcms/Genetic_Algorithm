...
def new_generation(networks, fittness, mutation_rate, n_children, layer_sizes):
    ...
    for i in range(n_children * len(networks)):
        p1 = weighted_choice(fittness)
        p2 = weighted_choice(fittness)
        parent1 = networks[p1]
        parent2 = networks[p2]
...
def save_population(networks, size, performance, layer_sizes, file_path):
    networks_data = {"size": size, "layer_sizes": layer_sizes, 'performance': performance, 'parametres': []}
